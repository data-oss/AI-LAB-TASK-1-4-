...
def hangman():
    ...
    def chose_word():
        easy_word=["apple","banana","cherry","date","elderberry"]
        medium_word=["kangaroo","lemur","monkey","narwhal","octopus"]
        hard_word=["elephant","giraffe","hippo","iguana","jaguar"]
        choice=input("Choose the level of difficulty: easy, medium, hard: ").lower()
        if choice=="easy":
            return random.choice(easy_word).lower(),len(hangman) - 3
        elif choice=="medium":
            return random.choice(medium_word).lower(),len(hangman) - 2
        elif choice=="hard":
            return random.choice(hard_word).lower(),len(hangman) - 1
        else:
            print("Invalid choice")
            return random.choice(medium_word).lower(),len(hangman) - 2
    play_again=True
